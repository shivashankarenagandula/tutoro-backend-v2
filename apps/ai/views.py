...
#
# The service area / subject / class lists are DELIBERATELY NOT here --
# those change whenever staff add a locality or subject in Django
# admin, and a static list here would silently go stale (this happened
# already: this constant listed 6 of the 8 real active areas, missing
# two that were added to the database after the constant was written).
# See _build_facts() below, which pulls live data at request time instead.
TUTORO_STATIC_FACTS = """
...
- Online tutoring is available to families anywhere, not just within
  the specific localities listed below.
...
def _build_facts():
    ...
    copy to drift out of sync in the first place.
    """
...
class FAQChatView(APIView):
    ...
    def post(self, request):
        ...
        try:
            answer = complete(system, question, max_tokens=500)
