...
class PasswordResetOTP(models.Model):
    """
    One-time code for the "forgot password" flow -- deliberately a
    separate model from EmailOTP rather than reusing it, even though
    the shape is identical, because the two codes must never be
    interchangeable: an email-verification code should never double
    as a password-reset code (that would let anyone who briefly
    intercepts a verification email also take over the account), and
    this flow has to work for a logged-OUT user identified only by
    email, unlike EmailOTP which requires request.user.
    """
...
class PasswordResetOTP(models.Model):
    ...
    def __str__(self):
        state = "used" if self.is_used else ("expired" if not self.is_valid() else "active")
        return f"Password reset OTP for {self.user.email} ({state})"
